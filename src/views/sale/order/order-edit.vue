...
import { getSalesOrderById } from '@/api/business/sales-order'
...
export default {
  ...
  async created() {
    ...
    await getSalesOrderById(this.$route.params.id).then(res => {
          ...
          column: [
            {
              ...
              click: (event, row) => {
                getProductInfoById(event).then(res => {
                  ...
                  row.customProductCode = data.customProductCode
                })
              }
            },
            {
