...
import {
  ...
  deleteSalesOrderByIds, procurement,
  ...
  methods: {
        ...
        },
        {
          ...
          button: [
            ...
            },
            {
              ...
              css: 'text',
              click: (index, row) => {
                deleteOutById(row.id).then(response => {
    ...
    handleSubmit() {
      ...
      if (canSubmit) {
        ...
        submitOutBatch(checkedRowIds).then(res => {
          const { msg, code } = res
          if (code === '100') {
            this.$modal.msgSuccess(msg)
          }
    ...
    handleDeliveryDateRangeChange(value) {
      if (value && value.length === 2) {
        this.queryForm.outStartDate = value[1]
        this.queryForm.outEndDate = value[2]
      } else {
        this.queryForm.outStartDate = ''
        this.queryForm.outEndDate = ''
      }
    },
    handleSalesReturn() {
      var checkedRows = this.$refs.tableList.checkedRows()
