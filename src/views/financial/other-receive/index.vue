  ...
  <div class="app-container">
    <el-form ref="queryForm" size="mini" :inline="true" :model="queryForm" v-if="showSearch">
      <el-form-item label="单据编号" prop="otherReceiveCode">
        <el-input v-model="queryForm.otherReceiveCode" clearable size="mini" placeholder="请输入单据编号" />
      </el-form-item>
      <el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      dataSource: '/api/financial-other-receive/list',
      tableColumnConfig: []
    }
  },
  async created() {
  ...
  methods: {
        ...
        },
        {
          ...
          button: [
            ...
            },
            {
              ...
              css: 'text',
              click: (index, row) => {
                submitOtherReceiveById(row.id).then(res => {
                  const { code, msg } = res
                  if (code === '100') {
            ...
            },
            {
              ...
              css: 'text',
              click: (index, row) => {
                deleteOtherReceiveById(row.id).then(response => {
    ...
    handleDel() {
      ...
      if (canDel) {
        ...
        deleteOtherReceiveByIds(ids).then(res => {
          const { msg, code } = res
          if (code === '100') {
            this.$modal.msgSuccess(msg)
            this.handleQuery()
    ...
    handleSubmit() {
      ...
      if (canSubmit) {
        ...
        submitOtherReceiveByIds(checkedRowIds).then(res => {
          const { code, msg } = res
          if (code === '100') {
            this.$modal.msgSuccess(msg)
            this.handleQuery()
