...
import { getInvoiceOrderById } from '@/api/business/invoice-order'
...
export default {
  ...
  data() {
    return {
      showForm: false,
      contentText: '销售开票修改',
      saveUrl: '/api/financial-invoice-order/update',
      submitUrl: '/api/financial-invoice-order/update-and-submit-single',
      canSubmit: true,
      collapseConfig: [
      ...
      javaCode: [],
      javaCodeConfig: {
        javaCodeNameList: ['UserBuilder', 'CustomerBuilder', 'ProductBuilder', 'CapitalAcountBuilder']
      }
    }
  },
  async created() {
    ...
    await this.init()
    await getInvoiceOrderById(this.$route.params.id).then(res => {
