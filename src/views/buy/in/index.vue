...
import { deleteOrderById, deleteOrderByIds, returnOrder, submitBatch } from '@/api/business/in'
import { procurementIn } from '@/api/business/procurement-order'
...
export default {
  ...
  data() {
    return {
      ...
      dataSource: '/api/procurement/in/list',
      tableColumnConfig: [],
      javaCode: [],
      javaCodeConfig: {
  ...
  methods: {
    ...
    handleDeliveryDateRangeChange(value) {
      if (value && value.length === 2) {
        this.queryForm.inStartDate = value[1]
        this.queryForm.inEndDate = value[2]
      } else {
        this.queryForm.inStartDate = ''
        this.queryForm.inEndDate = ''
      }
    },
    handleSubmit() {
      ...
      if (canSubmit) {
        ...
        submitBatch(checkedRowIds).then(res => {
          const { msg, code } = res
          if (code === '100') {
            this.$modal.msgSuccess(msg)
          }
    ...
    handleReturn() {
      ...
      if (canIn) {
        ...
        this.$modal.msgWarning('选中数据中有未审核通过数据，请检查后再次退货')
      }
    }
  }
}
