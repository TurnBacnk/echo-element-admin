...
import { getInOrderByCode, getInOrderById } from '@/api/business/in'
...
export default {
  ...
  data() {
    return {
      showForm: false,
      contentText: '审核采购入库单',
      saveUrl: '/api/procurement/in/update',
      submitUrl: '/api/procurement/update-and-submit',
      canSubmit: true,
      collapseConfig: [
      ...
      rules: {
        baseInfo: {
          ...
          inDate: [{ required: true, message: '请选择交货日期', trigger: 'blur' }],
  ...
  async created() {
    ...
    await getInOrderByCode(this.$route.params.code).then(res => {
