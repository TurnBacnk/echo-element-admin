...
import { getProcurementReturnOrderById } from '@/api/business/procurement-return'

export default {
  name: 'ProcurementOrderReturnEdit',
  components: { FormTable },
  data() {
    return {
      showForm: false,
      contentText: '修改采购退货单',
      saveUrl: '/api/procurement/return/update',
      submitUrl: '/api/procurement/return/update-and-submit',
      canSubmit: true,
      collapseConfig: [
  ...
  async created() {
    ...
    await getProcurementReturnOrderById(this.$route.params.id).then(res => {
